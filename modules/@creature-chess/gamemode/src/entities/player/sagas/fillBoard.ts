...
export const fillBoard = function*() {
	...
	yield takeEvery<FillBoardCommand>(FILL_BOARD_COMMAND, function*() {
		...
		while (true) {
			...
			const benchPiece = getMostExpensiveBenchPiece(benchSlice, pieceRegistry);

			if (!benchPiece) {
				return;
			}

			const destination = BoardSelectors.getFirstEmptySlot(boardSlice);

			if (!destination) {
				return;
			}

			const benchPiecePosition = benchSlice.getPiecePosition(benchPiece.id);
