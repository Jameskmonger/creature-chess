	...
	yield takeLatest(
		...
		function*({ payload: { pieceId } }) {
			...
			const matchingBoardPieces = getCombinablePieces(
				getPiecesForDefinition(boardSlice, pieceRegistry, targetDefinitionId),
				targetStage
			);

			const matchingBenchPieces = getCombinablePieces(
				getPiecesForDefinition(benchSlice, pieceRegistry, targetDefinitionId),
				targetStage
			);
			...
			if (matchingBoardPieces.length > 0) {
				...
				const piecePosition = boardSlice.getPiecePosition(pieceToReplace.id);
				...
				const piecePosition = benchSlice.getPiecePosition(piece.id);
