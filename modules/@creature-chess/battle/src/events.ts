...
export type BattleTurnEvent = {
	type: BATTLE_TURN_EVENT;
	payload: { turn: number; board: BoardState<PieceModel> };
};
export type BattleFinishEvent = {
	type: BATTLE_FINISH_EVENT;
	payload: { turns: number };
};

export type BattleEvent = BattleTurnEvent | BattleFinishEvent;

export const battleTurnEvent = (
	turn: number,
	board: BoardState<PieceModel>
): BattleTurnEvent => ({ type: BATTLE_TURN_EVENT, payload: { turn, board } });
export const battleFinishEvent = (turns: number): BattleFinishEvent => ({
	type: BATTLE_FINISH_EVENT,
	payload: { turns },
});
