...
async function createIndices(client: FaunaDBClient) {
	...
	try {
		...
		console.log(
			` - Created index '${INDEX_NAMES.USERS_BY_NICKNAME_UPPERCASE}'`
		);
		changesMade = true;
	} catch (e: any) {
	...
	try {
		await client.query(
			q.CreateIndex({
				name: INDEX_NAMES.USERS_BY_AUTH_ID,
				serialized: true,
				unique: true,
				source: q.Collection(COLLECTION_NAMES.USERS),
				terms: [
...
export const setupUserDatabase = async (
	...
	const indicesCreated = await createIndices(client);

	return changesMade || indicesCreated;
};
