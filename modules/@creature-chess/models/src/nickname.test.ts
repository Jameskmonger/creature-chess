...
describe("validateNicknameFormat", () => {
	describe.each(["bob_test", "some_cool_nick", "my _ name"])(
		"when nickname contains an underscore (%s)",
		(input) => {
	...
	describe.each(["bob", "a", "", null])(
		"when nickname is too short (%s)",
		(input) => {
	...
	describe.each(["a really really long nickname", "abcdefghijklmnopq"])(
		"when nickname is too long (%s)",
		(input) => {
	...
	describe.each(["jkm 1234", "Bob the Chess99"])(
