...
export const getBenchPieceForSlot = (state: PlayerPiecesState, slot: number): Piece => state.bench.pieces[slot] || null;
export const getBoardPieceForPosition =
  (state: PlayerPiecesState, x: number, y: number): Piece => state.board.pieces[state.board.piecePositions[`${x},${y}`]] || null;
export const getAllPieces = (state: PlayerPiecesState): Piece[] => [...Object.values(state.board.pieces), ...state.bench.pieces.filter(p => p !== null)];
export const getBoardPiecesForDefinition =
  (state: PlayerPiecesState, definitionId: number): Piece[] => Object.values(state.board.pieces).filter(p => p.definitionId === definitionId);
export const getBenchPiecesForDefinition =
  (state: PlayerPiecesState, definitionId: number): Piece[] => state.bench.pieces.filter(p => p && p.definitionId === definitionId);
