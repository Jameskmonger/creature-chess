...
mkdir -p ./bin ./bin/server ./bin/auth-server ./bin/models ./bin/data ./bin/shared

cp -r ./packages/server/package.json ./packages/server/lib/ ./packages/server/node_modules/ ./bin/server/ 2>/dev/null || :
