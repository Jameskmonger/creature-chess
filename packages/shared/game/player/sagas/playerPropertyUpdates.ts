...
import { BATTLE_UPDATED, ClearOpponentAction, CLEAR_OPPONENT, HEALTH_UPDATED, STATUS_UPDATED, STREAK_UPDATED, UpdateBattleAction, UpdateHealthAction, UpdateStatusAction, UpdateStreakAction } from "../../../player/playerInfo";
...
export const createPropertyUpdateRegistry = (sagaMiddleware: SagaMiddleware): PlayerPropertyUpdateRegistry => {
    ...
    sagaMiddleware.run(function*() {
        yield all([
            ...
            yield takeLatest<UpdateStatusAction>(
                STATUS_UPDATED,
                function*({ payload: { status } }) {
                    events.emit(PlayerPropertyUpdateEvent.UPDATE_STATUS, status);
                }
            ),
            yield takeLatest<UpdateBattleAction>(
                BATTLE_UPDATED,
