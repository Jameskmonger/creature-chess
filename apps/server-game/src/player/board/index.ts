...
const spectatePlayerBoard = function*(
	...
	if (initialMatch) {
		registry.send("matchBoardUpdate", {
			...
			board: serialiseBoard(initialMatch.getBoardForPlayer(playerId), pieceRegistry),
		});
	...
	yield all([
		call(function*() {
			while (true) {
				...
				if (match) {
					registry.send("matchBoardUpdate", {
						turn: null,
						board: serialiseBoard(match.getBoardForPlayer(playerId), pieceRegistry),
					});
				}
		...
		takeLatest(
			...
			function*() {
				...
				registry.send("boardUpdate", serialiseBoard(boardSlice, pieceRegistry));
			}
		),
		takeLatest(
			[
			...
			function*() {
				...
				registry.send("benchUpdate", serialiseBoard(benchSlice, pieceRegistry));
			}
		),
	]);
};
